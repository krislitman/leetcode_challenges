...
class Solution(object):
    def removeElement(self, nums, val):
        newArray = list(map(lambda x: x if x != val else 0, nums))
        while 0 in newArray:
            newArray.remove(0)
        return newArray
...
solution = Solution()
attempt1 = solution.removeElement(nums=[3, 2, 2, 3], val=3)
# attempt2 = solution.removeElement(nums = [0,1,2,2,3,0,4,2], val = 2)
